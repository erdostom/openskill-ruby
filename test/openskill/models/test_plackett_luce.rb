...
module OpenSkill
  module Models
    class TestPlackettLuce < Minitest::Test
      ...
      def test_rating_hash
        r1 = @model.create_rating(mu: 30.0, sigma: 5.0)
        r2 = @model.create_rating(mu: 30.0, sigma: 5.0)

        # Hash should be based on ID, so different objects have different hashes
        refute_equal r1.hash, r2.hash

        # But equality is based on mu and sigma
        assert_equal r1, r2
      end
      ...
      def test_calculate_ratings_with_limit_sigma
        ...
        team1 = [model.create_rating]
        team2 = [model.create_rating, model.create_rating]
        team3 = [model.create_rating, model.create_rating, model.create_rating]
      ...
      def test_predict_win_detects_impostor
        a1 = @model.create_rating
        a2 = @model.create_rating(mu: 32.444, sigma: 5.123)

        probs = @model.predict_win_probability([[a1], [a1], [a1], [a2], [a1]])
        # a2 (stronger player) should have highest probability
        assert_equal probs[3], probs.max
      end
